...
def dynamic_save_to_csv(data, filename, company_name, input_company_name, membership_column, membership_value, timestamp, redirected_to_URL ,fill_value="-*"):
    """
    ...
    if os.path.isfile(filename):
        ...
        df = pd.DataFrame(columns=["Input Company Name", "Company Name", membership_column, "Time Stamp" , "Redirected to URL"])
    ...
    for role_entry in data:
        role_name = role_entry.get("Role", "Unknown Role") if isinstance(role_entry, dict) else str(role_entry)
        count = role_entry.get("Count", fill_value) if isinstance(role_entry, dict) else fill_value
...
def process_about_data(about_data_file, output_file='about_data.csv'):
    ...
    for about_data in data:
        associated_members = re.sub(r"[^\d]", "", about_data.get("No. of associated members", "")) or "-*"
        ...
        rows.append({
            ...
            "Time Stamp": about_data.get("Time Stamp", datetime.datetime.now().isoformat()),
        ...
        for loc in locations:
            if isinstance(loc, dict):
                row = {
                    ...
                    "Count": loc.get("Count", "-*"),
                    "Time Stamp": entry.get("Time Stamp", datetime.datetime.now().isoformat()),
                    "Redirected to URL" : entry.get("Redirected to URL", "-*")
                }
                if include_quantum:
                    row["Quantum Associated Members"] = entry.get("Quantum Associated Members", "-*")
                else:
...
def main():
    ...
    about_data_file = 'about_data.json'
    what_they_do_file_with_quantum = 'what_they_do_data_with_quantum.json'
    what_they_do_file_without_quantum = 'what_they_do_data_without_quantum.json'
    where_they_live_file_with_quantum = 'where_they_live_with_quantum_data.json'
    where_they_live_file_without_quantum = 'where_they_live_without_quantum_data.json'
